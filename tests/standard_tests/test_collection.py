...
class TestAddItem:
    def test_add_item_returns_id(self, collection):
        vec = np.random.rand(DIM).astype(np.float32)
        ret = collection.add_item(vec, id=1, buffer_size=False)
        collection.commit()
        assert ret == 1

    def test_add_item_with_field(self, collection):
        vec = np.random.rand(DIM).astype(np.float32)
        collection.add_item(vec, id=2, field={"key": "val"}, buffer_size=False)
        collection.commit()
        assert collection.is_id_exists(2)

    def test_add_item_buffered(self, collection):
        for i in range(5):
            collection.add_item(
                np.random.rand(DIM).astype(np.float32), id=i, buffer_size=10
            )
        collection.commit()
...
class TestBulkAdd:
    def test_bulk_add_items(self, collection):
        items = [
            (np.random.rand(DIM).astype(np.float32), i, {"tag": str(i)})
            for i in range(10)
        ]
        with collection.insert_session() as session:
            ids = session.bulk_add_items(items, enable_progress_bar=False)
        assert len(ids) == 10
        assert collection.shape[0] == 10

    def test_bulk_add_items_no_fields(self, collection):
        items = [(np.random.rand(DIM).astype(np.float32), i) for i in range(5)]
        with collection.insert_session() as session:
            ids = session.bulk_add_items(items, enable_progress_bar=False)
        assert len(ids) == 5
...
class TestSoftDelete:
    def test_delete_items(self, populated_collection):
        populated_collection.delete_items([0, 1])
    ...
    def test_restore_items(self, populated_collection):
        populated_collection.delete_items([2, 3])
        populated_collection.restore_items([2])
        deleted = populated_collection.list_deleted_ids()
        assert 2 not in deleted
        assert 3 in deleted

    def test_deleted_excluded_from_stats(self, populated_collection):
        populated_collection.delete_items([0, 1, 2])
        s = populated_collection.stats()
    ...
    def test_delete_then_restore_all(self, populated_collection):
        ...
        populated_collection.delete_items(ids_to_delete)
        populated_collection.restore_items(ids_to_delete)
        assert populated_collection.list_deleted_ids() == []
...
class TestCompact:
    ...
    def test_compact_clears_tombstone(self, populated_collection):
        populated_collection.delete_items([0])
...
class TestCommit:
    def test_commit_after_add(self, collection):
        collection.add_item(np.random.rand(DIM).astype(np.float32), id=1, buffer_size=False)
        collection.commit()
...
class TestEmptyCollection:
    ...
    def test_delete_items_nonexistent_ids_adds_to_tombstone(self, collection):
        collection.delete_items([999, 1000])
        deleted = collection.list_deleted_ids()
        assert 999 in deleted and 1000 in deleted


class TestInsertSession:
    def test_session_commits_on_exit(self, collection):
        with collection.insert_session() as session:
            session.add_item(np.ones(DIM, dtype=np.float32), id=77)
        assert collection.is_id_exists(77)

    def test_session_bulk_commits_on_exit(self, collection):
        items = [
            (np.random.rand(DIM).astype(np.float32), i, {"v": i})
            for i in range(5)
        ]
        with collection.insert_session() as session:
    ...
    def test_session_multiple_add_items(self, collection):
        with collection.insert_session() as session:
            for i in range(10):
...
class TestDuplicateId:
    def test_add_duplicate_id_is_rejected(self, collection):
        ...
        with pytest.raises(RuntimeError, match="already exists"):
            collection.add_item(v2, id=1, buffer_size=False)
        assert collection.is_id_exists(1)
...
class TestUpsert:
    def test_upsert_item_updates_existing_without_growing_shape(self, collection):
        v1 = np.ones(DIM, dtype=np.float32)
        v2 = np.zeros(DIM, dtype=np.float32)
        collection.add_item(v1, id=10, field={"tag": "old"}, buffer_size=False)
        collection.upsert_item(v2, id=10, field={"tag": "new"})
    ...
    def test_upsert_items_mixes_update_and_insert(self, collection):
        collection.add_item(np.ones(DIM, dtype=np.float32), id=1, field={"tag": "old"}, buffer_size=False)
        ids = collection.upsert_items([
            (np.zeros(DIM, dtype=np.float32), 1, {"tag": "new"}),
            (np.ones(DIM, dtype=np.float32) * 2, 2, {"tag": "inserted"}),
    ...
    def test_upsert_without_field_preserves_existing_fields(self, collection):
        collection.add_item(np.ones(DIM, dtype=np.float32), id=5, field={"tag": "keep"}, buffer_size=False)
        collection.upsert_item(np.zeros(DIM, dtype=np.float32), id=5)
...
class TestEdgeCases:
    ...
    def test_add_item_no_field(self, collection):
        vec = np.random.rand(DIM).astype(np.float32)
        collection.add_item(vec, id=42, buffer_size=False)
        collection.commit()
    ...
    def test_delete_already_deleted_is_idempotent(self, populated_collection):
        populated_collection.delete_items([5])
        populated_collection.delete_items([5])
        deleted = populated_collection.list_deleted_ids()
