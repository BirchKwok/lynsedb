...
class FieldsMapper:
    ...
    def _single_encode(self, single_data):
        if single_data not in self.fields_values:
            ...
            self.fields[self.last_id] = single_data
            self.fields_mapper[single_data] = self.last_id
            return self._get_next_id()
        else:
            return self.fields_mapper[single_data]
    ...
    def save(self, filepath):
        ...
        try:
            with open(filepath, 'wb') as f:
                f.write(msgpack.packb([self.fields, self.fields_mapper, self.last_id]))
        except IOError as e:
    ...
    def load(self, filepath):
        ...
        try:
            with open(filepath, 'rb') as f:
                self.fields, self.fields_mapper, self.last_id = msgpack.unpackb(f.read(), strict_map_key=False,
                                                                                raw=False, use_list=False)
            self.fields_values = set(self.fields_mapper.keys())
        except IOError as e:
