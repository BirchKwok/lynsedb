...
def ip(vec1, vec2, n, use_simd=None):
    ...
    if should_use_simd:
        try:
            ...
            # Fall back to other implementations if SimSIMD fails
            if config.LYNSE_SIMSIMD_AUTO_FALLBACK:
                _log_fallback(f"SimSIMD error: {str(e)}", "ip")
                should_use_simd = False  # ensure fallback path below is taken
            else:
                raise
